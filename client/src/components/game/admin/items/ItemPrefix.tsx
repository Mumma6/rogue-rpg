...
const ItemPrefix = ({ toggle }: any) => {
  const dispatch = useDispatch()
  const currentItemPrefixs = useSelector(
    (state: State) => state.admin.itemPrefixs
  )

  useEffect(() => {
    dispatch(getAllItemPrefixs())
  }, [dispatch])
  ...
  interface ItemPrefix extends ObjectAlias {
    _id: string
  }

  const { name, levelRangeFrom, levelRangeTo, frequency, stats } = formData

  return (
    <div className="container-fluid" style={{ marginTop: 40, width: 500 }}>
      <p className="lead">
        <i className="fas fa-magic" /> Create item prefix
      </p>
      <Form
        className="form"
      ...
      >
        ...
        <Form.Group>
          ...
          <Form.Control
            ...
            value={stats}
            onChange={(evt: ChangeEvent<HTMLInputElement>) => handleChange(evt)}
            isInvalid={checkIsInvalid(errors, 'stats')}
          />
        </Form.Group>
        <input
          type="submit"
          className="btn btn-success"
          value="Create item prefix"
        />
        <input
          type="button"
      ...
      <h1>Item prefixs already created</h1>
      {currentItemPrefixs.map((item: ItemPrefix) => (
        <div>
          <div>{item.name}</div>
          <button
            type="button"
            className="btn btn-danger"
            onClick={() => dispatch(deleteItemPrefix({ id: item._id }))}
          >
